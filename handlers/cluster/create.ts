...
export const createHandler: RouteHandler = async (req, res) => {
  try {
    ...
    const { name, type, environment, region, cidr, code_bucket, bref_layer_arn, api_domain, certificate_arn } = validation.data;
    ...
    res.status(201).json({
      ...
      data: {
        ...
        bref_layer_arn: result.data?.bref_layer_arn,
        api_domain: result.data?.api_domain,
        certificate_arn: result.data?.certificate_arn,
        status: result.data?.status,
